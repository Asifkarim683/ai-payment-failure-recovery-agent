...
`;
  ...
  try {
    ...
    // Graceful deterministic fallback if network / key error occurs
    return {
...
`;
  ...
  try {
    const raw = await callGemini(prompt, {
      systemInstruction: "You are the Recoverly AI Copilot. Always provide helpful, quantitative revenue insights in strict JSON format.",
      responseMimeType: "application/json",
    });
    const parsed = JSON.parse(raw);
    return {
